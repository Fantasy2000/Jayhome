(function(){
  ...
  function buildModal(){
    ...
    modal.innerHTML = `
      ...
      <div class="mb-panel">
        <div class="mb-header">
          ...
          <button class="mb-close">&times;</button>
        </div>
        <div class="mb-body">
          <div class="mb-list"></div>
          <div class="mb-compose">
            <img class="mb-avatar" src="${avatarUrl('me')}" alt="avatar"/>
            <textarea class="mb-input" placeholder="写下你的留言...（至少5个字）" maxlength="50"></textarea>
    ...
    $('.mb-send', modal).addEventListener('click', async ()=>{
      ...
      try{
        ...
        ta.value=''; await loadThreads(1);
      }catch(e){ alert(e.message||'发送失败'); }
    });
  }

  function openModal(){ buildModal(); const modal = $('#messageBoardModal'); if(modal){ modal.classList.add('active'); loadThreads(1); }}
  ...
  async function loadThreads(page = 1){
    ...
    try{
      const resp = await fetch(`${API_URL}?action=threads&limit=${COMMENTS_PER_PAGE}&page=${page}&_t=${Date.now()}`, { credentials:'same-origin' });
  ...
  function renderPagination(){
    ...
    if(totalPages <= 1){
      const existingPagination = $('.mb-pagination', list);
      if(existingPagination) existingPagination.remove();
      return;
    }
    
    let pagination = $('.mb-pagination', list);
    if(!pagination){
      pagination = document.createElement('div');
      pagination.className = 'mb-pagination';
      list.appendChild(pagination);
    }
    
    let html = '';
    
    html += `<button ${currentPage === 1 ? 'disabled' : ''} onclick="window.loadMessageBoardPage(${currentPage - 1})">‹</button>`;
    
    for(let i = 1; i <= totalPages; i++){
      if(i === 1 || i === totalPages || (i >= currentPage - 1 && i <= currentPage + 1)){
        html += `<button class="${i === currentPage ? 'active' : ''}" onclick="window.loadMessageBoardPage(${i})">${i}</button>`;
      } else if(i === currentPage - 2 || i === currentPage + 2){
        html += `<span class="mb-pagination-info">...</span>`;
      }
    }
    
    html += `<button ${currentPage === totalPages ? 'disabled' : ''} onclick="window.loadMessageBoardPage(${currentPage + 1})">›</button>`;
    
    html += `<span class="mb-pagination-info">${currentPage}/${totalPages}</span>`;
    
    pagination.innerHTML = html;
  }

  window.loadMessageBoardPage = function(page){
    ...
    const header = `
      <div class="mb-item">
        ...
        <div class="mb-item-right">
          ...
          <div class="mb-text-container">
            <div class="mb-text">${escapeHtml(item.text||'')}</div>
            <button class="mb-copy-btn" data-text="${escapeHtml(item.text||'')}" title="复制文本">
          ...
          <div class="mb-reply-box" data-for="${item.id}" hidden>
            <div class="mb-reply-compose">
              <textarea class="mb-reply-input" placeholder="回复...（至少5个字）" maxlength="50"></textarea>
      ...
      <div class="mb-reply">
        ...
        <div class="mb-reply-body">
          ...
          <div class="mb-text-container">
            ...
            <button class="mb-copy-btn" data-text="${escapeHtml(r.text||'')}" title="复制文本">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="16" height="16">
                <rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect>
                <path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path>
              </svg>
